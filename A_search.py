...
class A_search:
    ...
    def astar_path_constrained(self, weight="Cost", *, cutoff=None, constrain=True, service_DR=0,cap_check=True):
        ...
        while queue:
            ...
            if curnode == goal:
                ...
                return path,dist,{"enqueued_size":len(enqueued), "pushed count":pushed_count, "generated_count":generated_count,"expanded_count":expanded_count}
    ...
    def astar_path_constrained_flat(self, weight="Cost", *, test_name = None, cutoff=None, constrain=True, service_DR=0,cap_check=True,g_n=1):
        """Returns a list of nodes in a shortest path between source and goal
        using the A* ("A-star") algorithm.
    ...
    def astar_path_constrained_node_cap(self, weight="Cost", *, cutoff=None, constrain=False, service_DR=0,cap_check=True):
            """Returns a list of nodes in a shortest path between source and goal
            using the A* ("A-star") algorithm.
            ...
            while queue:
                ...
                for neighbor, w in G_succ[curnode].items():
                    ...
                    # ---- node capacity constraint on vertical edges, per-path usage ----
                    node_id_cur = G.nodes[curnode]["node_id"]
                    node_id_nei = G.nodes[neighbor]["node_id"]
                    layer_cur = G.nodes[curnode]["Layer"]
                    layer_nei = G.nodes[neighbor]["Layer"]

                    is_vertical = (node_id_cur == node_id_nei) and (layer_nei == layer_cur + 1) 


                    if cap_check and is_vertical:  # added this
                        if(self.print_):
                            print("gateway node(current,gateway):", (curnode, neighbor))
                        # Function placed at the target layer (layer_nei corresponds to self.functions[layer_nei - 1])
                        func_id = self.functions[layer_nei - 1]["ID"]  
                        required_cap = function_req.get(func_id, 0)

                        phys_id = node_id_nei 
                        total_cap = base_capacity.get(phys_id, float("inf"))  

                        # compute how much capacity this physical node has already used
                        # along the current subpath (source -> curnode)             
                        used_cap = 0.0  
                        tmp = curnode 
                        while tmp is not None:  
                            tmp_data = G.nodes[tmp]  
                            tmp_phys = tmp_data["node_id"]  
                            tmp_layer = tmp_data["Layer"] 

                            if tmp_phys == phys_id and tmp_layer > 0:  
                                parent_tmp = explored.get(tmp) 
                                if parent_tmp is not None:  # added this
                                    parent_data = G.nodes[parent_tmp]  # added this
                                    parent_phys = parent_data["node_id"]  # added this
                                    parent_layer = parent_data["Layer"]  # added this
                                    # vertical move into tmp? then a function was placed there  # added this
                                    if parent_phys == phys_id and tmp_layer == parent_layer + 1:  # added this
                                        tmp_func_id = self.functions[tmp_layer - 1]["ID"]  # added this
                                        used_cap += function_req.get(tmp_func_id, 0)  # added this
                            tmp = explored.get(tmp)

                        remaining_cap = total_cap - used_cap 
                        if self.print_:
                            print(
                                f"phys node {phys_id}: total_cap={total_cap}, used_cap={used_cap}, "
                                f"remaining_cap={remaining_cap}, required={required_cap}"
                            )  # added this

                        if remaining_cap < required_cap:
                            if self.print_:
                                print(
                                    f"X skipped vertical edge (u,v): ({curnode},{neighbor}) "
                                    f"remaining {remaining_cap} < required {required_cap}"
                                )  # added this
                            continue  # added this
                    # ---- end node capacity constraint ----
    ...
    def astar_path_constrained_LLM(self, weight="Cost", *,t_n_weight, test_name=None, cutoff=None, constrain=False, service_DR=0,cap_check=False):
            """Returns a list of nodes in a shortest path between source and goal
            using the A* ("A-star") algorithm.
    ...
    # def astar_path_constrained_LLM_FOCAL(self, weight="Cost", *,t_n_weight, test_name=None, cutoff=None, constrain=False, service_DR=0,cap_check=False):
    #         """Returns a list of nodes in a shortest path between source and goal
    #         using the A* ("A-star") algorithm.
    ...
    #         # The queue stores priority, node, cost to reach, parent, and Layer, true_node_id
    #         # Uses Python heapq to keep in priority order.
    #         # Add a counter (next(c)) to the queue to prevent the underlying heap from
    #         # attempting to compare the nodes themselves. The hash breaks ties in the
    #         # priority and is guaranteed unique for all nodes in the graph.
    #         c = count()

    #         entries = {}
    #         def add(node, f, g, parent):
    #             if node in entries:
    #                 self.queue.remove(entries[node])
    #             entry = (f, next(c), node, g, parent)
    #             print("adding node:", node, "with entry:", entry)
    #             entries[node] = entry
    #             self.queue.add(entry)

    #         def pop_min():
    #             f, _, node, g, parent = self.queue.pop(0)
    #             del entries[node]
    #             return f, node, g, parent

    #         def remove(node):
    #             print("removing node:", node)
    #             entry = entries.pop(node)
    #             print("entry to remove:", entry)
    #             self.queue.remove(entry)

    #         def update_lower_bound(old_bound, new_bound):
    #             for f, _, node, g, parent in self.queue:
    #                 if old_bound < f <= new_bound:
    #                     heappush(
    #                         self.FOCAL,
    #                         (f + t_n_weight * self.h(node, self.s_target), next(c), node, g, parent)
    #                     )

            
    #         # FOCAL waypoint based heurisitc, , source, f(n) for the node, parent
    #         w_focal = 1.1
    #         T_n = self.h(source, goal) + self.h(source, self.s_target)
    #         self.FOCAL = [(T_n, next(c), source, 0, None)]

            
    #         # add node to OPEN
    #         # self.queue = [(0, next(c), source, 0, None)] 
    #         self.queue = SortedList()
    #         add(source,self.h(source, goal), 0, None)
    ...
    #             # FOCAL: get the min f(n) in the queue
    #             f_min = self.queue[0][0]

    #             # FOCAL get the top node in the FOCAL Queue
    #             Focal_h_n ,next_c, curnode, dist, parent = heappop(self.FOCAL)
    #             print("curnode: ", curnode, "and current Queue: ", self.queue)
    #             remove(curnode)

    #             print("current == goal? ",curnode == goal)
    #             if curnode == goal:
    #                 if self.print_:
    #                     print(f"path found with total cost: {dist}")
    #                     print(f"source {source}, and source: {goal}")
    ...
    #                 if neighbor == self.s_target and self.goal != self.s_target :
    #                     self._update_target()
    #                     self._update_queue_FOCAL(h_T_dist,t_n_weight)
    #                     if (self.print_):
    #                         print(neighbor, self.s_target)
    ...
    #                 # FOCAL: add nieghbor to queue. if f(n) less than fmin*w 
    #                 add(neighbor, ncost + h, ncost, curnode)
    #                 print("adding neighbor: ", neighbor, "with f(n): ", ncost + h, "and f_min*w_focal: ", w_focal*f_min)
    #                 print("adding node: ", neighbor)
    #                 if ncost + h <= w_focal*f_min:
    #                     heappush(self.FOCAL, (ncost + h + t_n_weight*self.h(neighbor, self.s_target), next(c), neighbor, ncost, curnode))
    #                 # heappush(self.queue, (ncost + h + t_n_weight*self.h(neighbor, self.s_target), next(c), neighbor, ncost, curnode))
    ...
    #                 # update boundry if needed
    #                 if self.queue:
    #                     new_f_min = self.queue[0][0]
    #                     if f_min < new_f_min:
    #                         old_bound = w_focal * f_min
    #                         new_bound = w_focal * new_f_min
    #                         update_lower_bound(old_bound, new_bound)

    #         raise nx.NetworkXNoPath(f"Node {goal} not reachable from {source}")
    def astar_path_constrained_LLM_FOCAL(
        self,
        weight="Cost",
        *,
        t_n_weight,
        ...
        """
        Focal-Search variant of the LLM-waypoint-biased A*.

        Two heaps, both using lazy deletion:
        • OPEN  – sorted by f(n) = g(n) + h(n, goal)        [admissible]
        • FOCAL – sorted by d(n) = h(n, s_target)            [waypoint bias]

        FOCAL contains only the subset of OPEN with f(n) <= w_focal * f_min.
        We always expand from FOCAL, giving bounded sub-optimality (factor w_focal)
        while letting the LLM waypoint heuristic steer within that bound.
        """
        ...
        service_DR = self.service_DR
        function_req = {
            func["ID"]: func["requirements"] * self.service_DR for func in self.functions
        }
        ...
        # ------------------------------------------------- best-known g per node
        # This is the single source of truth for "is this entry stale?"
        best_g = {}  # node -> best g(n) found so far
        parent_of = {}  # node -> parent on the best-g path

        # ------------------------------------------------------------ OPEN heap
        # entry: (f, tiebreaker, node, g, parent)
        OPEN = []
        # ----------------------------------------------------------- FOCAL heap
        # entry: (focal_priority, tiebreaker, node, g, parent)
        FOCAL = []

        # helpers ---------------------------------------------------------------
        def f_val(g, node):
            """Admissible f = g + h(node, goal)."""
            return g + self.h(node, goal)

        def focal_priority(node):
            return g + self.h(node, goal) + self.h(node, self.s_target)

        def open_push(node, g, parent):
            f = f_val(g, node)
            heappush(OPEN, (f, next(c), node, g, parent))

        def focal_push(node, g, parent):
            fp = focal_priority(node)
            heappush(FOCAL, (fp, next(c), node, g, parent))

        def get_f_min():
            """Peek at the best f in OPEN, skipping stale entries."""
            while OPEN:
                f, _, node, g, _ = OPEN[0]
                # stale: a strictly better g was found after this was pushed
                if node in best_g and g > best_g[node]:
                    heappop(OPEN)
                    continue
                # stale: node already expanded
                if node in explored:
                    heappop(OPEN)
                    continue
                return f
        ...
        # ------------------------------------------------- seed source into both
        h_source = self.h(source, goal)
        best_g[source] = 0
        parent_of[source] = None
        open_push(source, 0, None)
        focal_push(source, 0, None)

        enqueued = {}  # node -> (g, h) for the best path seen so far
        explored = {}  # node -> parent  (only after expansion)

        # ============================================================ main loop
        while OPEN:
            # 1. get current f_min from OPEN (skip stale) ----------------------
            f_min = get_f_min()
            if f_min is None:
                break

            # 2. pop from FOCAL (skip stale / already explored) ----------------
            while FOCAL:
                fp, _, curnode, dist, parent = heappop(FOCAL)

                # stale? – a better g was found after this entry was pushed
                if curnode in best_g and dist > best_g[curnode]:
                    continue
                # already expanded with equal-or-better cost?
                if curnode in explored:
                    continue
                # valid entry
                break
            else:
                # FOCAL exhausted but OPEN is not – this can happen if the
                # bound is very tight; widen or fall back to OPEN expansion
                # (shouldn't normally happen with correct bound maintenance)
                break
            ...
            if curnode == goal:
                if self.print_:
                    ...
                    print(
                        f"generated: {len(enqueued) + 1}, expanded: {expanded_count}"
                    )
                ...
                return (
                    ...
                    {
                        "enqueued_size": len(enqueued),
                        "pushed count": pushed_count,
            ...
            for neighbor, w in G_succ[curnode].items():
                ...
                if neighbor == self.s_target and self.goal != self.s_target:
                    self._update_target()
                    if self.print_:
                        print("waypoint advanced to", self.s_target)

                    # Rebuild FOCAL with the new waypoint's heuristic.
                    # FOCAL is a small subset of OPEN (only nodes within
                    # w * f_min), so this is cheap. OPEN is untouched.
                    cur_f_min = get_f_min()
                    if cur_f_min is not None:
                        FOCAL.clear()
                        bound = w_focal * cur_f_min
                        for entry in OPEN:
                            f_n, _, nd, g_n, par_n = entry
                            # skip stale / already expanded
                            if nd in best_g and g_n > best_g[nd]:
                                continue
                            if nd in explored:
                                continue
                            if f_n <= bound:
                                focal_push(nd, g_n, par_n)
                ...
                # skip if we already know a better or equal path
                if neighbor in best_g and best_g[neighbor] <= ncost:
                    continue
                ...
                # ---- this is a new best path to neighbor ----
                best_g[neighbor] = ncost
                parent_of[neighbor] = curnode
                enqueued[neighbor] = ncost, h
                G.nodes[neighbor]["status"] = "enqueued"

                # always add to OPEN
                open_push(neighbor, ncost, curnode)

                # add to FOCAL only if within the sub-optimality window
                if ncost + h <= w_focal * f_min:
                    focal_push(neighbor, ncost, curnode)

                pushed_count += 1
                if len(OPEN) > max_queue_size:
                    max_queue_size = len(OPEN)

            # 5. update lower bound (AFTER all neighbors) ----------------------
            new_f_min = get_f_min()
            if new_f_min is not None and new_f_min > f_min:
                old_bound = w_focal * f_min
                new_bound = w_focal * new_f_min
                # scan OPEN for nodes that now qualify for FOCAL
                for entry in OPEN:
                    f_n, _, node, g, par = entry
                    # skip stale
                    if node in best_g and g > best_g[node]:
                        continue
                    if node in explored:
                        continue
                    # was outside old window, now inside new window
                    if old_bound < f_n <= new_bound:
                        focal_push(node, g, par)

        raise nx.NetworkXNoPath(f"Node {goal} not reachable from {source}")



    def astar_path_constrained_LLM_FOCAL_v2(self, weight="Cost", *,t_n_weight,f_N, w_focal=1.1, test_name=None, cutoff=None, constrain=False, service_DR=0,cap_check=False):
            """Returns a list of nodes in a shortest path between source and goal
            using the A* ("A-star") algorithm.
            ...
            def focal_priority(g, node):
                """Secondary heuristic – distance to current LLM waypoint."""

                if f_N == "t":
                    return self.h(node, self.s_target)
                elif f_N == "g_t":
                    return g + self.h(node, self.s_target)
                elif f_N == "g_h_t":
                    return g + self.h(node, goal) + self.h(node, self.s_target)

            def open_push(node, g, parent):
                f = f_val(g, node)
                heappush(self.queue, (f, next(c), node, g, parent))

            def focal_push(node, g, parent):
                fp = focal_priority(g, node)
                heappush(FOCAL, (fp, next(c), node, g, parent))
            ...
            while FOCAL:
                ...
                # Get f_min for QQ_FOCAL
                f_min = get_f_min()

                # pop the top most element from FOCAL and OPEN
                fp, _, curnode, dist, parent = heappop(FOCAL)
                # # Pop the smallest item from queue.
                # _, __, curnode, dist, parent = heappop(self.queue)
                ...
                for neighbor, w in G_succ[curnode].items():
                    ...
                    heappush(self.queue, (ncost + h, next(c), neighbor, ncost, curnode))
                ...
                # QQ_FOCAL
                if new_waypoint:
                    FOCAL = self._update_queue_FOCAL(h_T_dist,FOCAL,t_n_weight)

                # TODO update the lower bound / based on new smallest value in OPEN
                new_f_min = get_f_min()
                if new_f_min is not None and new_f_min > f_min:
                    old_bound = w_focal * f_min
                    new_bound = w_focal * new_f_min
    ...
    def astar_path_constrained_LLM_mod(self, weight="Cost", *, cutoff=None, constrain=False, service_DR=0,cap_check=False):
            """Returns a list of nodes in a shortest path between source and goal
            using the A* ("A-star") algorithm.
            ...
            self.s_target = self.target_list[1]
            self.i = 1
            if not self.landmark_heuristic:
                h_T_dist = self.hops_to_set()
            else:
                self.get_min_edge_delay()
                h_T_dist = None
            ...
            while self.queue:
                ...
                cur_layer = get_layer(curnode)  # or pass None if strings
                tgt_layer = get_layer(self.s_target)

                if (
                    tgt_layer is not None
                    and cur_layer is not None
                    and cur_layer > tgt_layer
                    and self.goal != self.s_target
                ):
                    self._update_target()
                    self._update_queue(h_T_dist)
                    if self.print_:
                        print("Advanced target because frontier reached next layer:",
                            "current=", curnode, "new_target=", self.s_target)
                ...
                for neighbor, w in G_succ[curnode].items():
                    ...
                    # ASTAR
                    enqueued[neighbor] = ncost, h
                    if (self.landmark_heuristic):
                        heappush(self.queue, (ncost + h + self.h(neighbor, self.i), next(c), neighbor, ncost, curnode))
                    else:
                        heappush(self.queue, (ncost + h + h_T_dist[self.i][neighbor], next(c), neighbor, ncost, curnode))
    ...
    def astar_path_constrained_LLM_node_cap(self, weight="Cost", *, cutoff=None, constrain=True, service_DR=0, cap_check=True):
        """
        """
        ...
        source = self.source
        G = self.graph
        # function_req = [func['requirements']*service_DR for func in self.functions]  # changed this (old line)
        function_req = {func['ID']: func['requirements'] * self.service_DR for func in self.functions}
        ...
        if not self.landmark_heuristic:
            self.get_min_edge_delay()
            h_T_dist = [i for i in self.hops_to_set()]
        else:
            h_T_dist = None
        expanded_count = 0
        ...
        weight = _weight_function(G, 'Delay')
        ...
        while self.queue:
            ...
            for neighbor, w in G_succ[curnode].items():
                ...
                if neighbor in enqueued:
                    ...
                    h = self.h(neighbor, goal)
                if(self.print_):
                    print("neighbors:", neighbor, "with h(n):", h, "and g(n):", ncost, "and target distance", h_T_dist[self.i][neighbor])
                if cutoff and ncost + h > cutoff:
                    continue
